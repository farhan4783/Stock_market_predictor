...
const updateProgressAfterQuiz = (moduleId, xpReward, passed) => {
    try {
        const saved = localStorage.getItem(PROGRESS_KEY);
        const progress = saved ? JSON.parse(saved) : {};
        const modId = parseInt(moduleId);
        let { unlockedModules = [1], completedModules = [], badges = ['First Steps'], xp = 0 } = progress;

        if (passed) {
            // Unlock next module
            const next = modId + 1;
            if (!unlockedModules.includes(next) && next <= 3) {
                unlockedModules = [...unlockedModules, next];
            }
            if (!completedModules.includes(modId)) {
                completedModules = [...completedModules, modId];
            }
            // Award badges
            if (modId === 1 && !badges.includes('Module 1 Complete')) badges = [...badges, 'Module 1 Complete'];
            if (modId === 2 && !badges.includes('Chart Master')) badges = [...badges, 'Chart Master'];
            if (completedModules.length >= 3 && !badges.includes('Quiz Champion')) badges = [...badges, 'Quiz Champion'];

            xp += xpReward;
        }

        localStorage.setItem(PROGRESS_KEY, JSON.stringify({ ...progress, unlockedModules, completedModules, badges, xp }));
    } catch { }
};
                ...
                <div className="max-w-2xl mx-auto">
                    ...
                    >
                        ...
                        <div className="mb-6">
                            ...
                            <p className="text-gray-400">{scorePercentage.toFixed(0)}% Score</p>
                        </div>

                        {passed ? (
                            <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 mb-6">
                                <p className="text-green-400 font-medium">
                                    ✅ Passed! +{quiz.xp_reward} XP earned
                                </p>
                                {parseInt(moduleId) < 3 && (
                                    <p className="text-emerald-300 text-sm mt-2 font-semibold">
                                        🔓 Module {parseInt(moduleId) + 1} is now unlocked!
                                    </p>
                                )}
                                <p className="text-gray-400 text-sm mt-1">Passing score: {quiz.passing_score}%</p>
                            </div>
                        ) : (
                            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-6">
                                ...
                                <p className="text-gray-400 text-sm mt-2">Review the lessons and try again!</p>
                            </div>
                        )}
                        ...
                        <div className="flex gap-4 justify-center flex-wrap">
                            ...
                            <Link to="/learner">
                                <motion.button
                                    whileHover={{ scale: 1.03 }}
                                    whileTap={{ scale: 0.97 }}
                                    className="px-6 py-3 bg-gradient-to-r from-neon-blue to-neon-purple rounded-lg text-white font-bold hover:shadow-cyan-500/30 transition-all"
                                >
                                    Back to Dashboard
                                </motion.button>
                            </Link>
                    ...
                    {!showExplanation ? (
                        <motion.button
                            ...
                            className={`px-6 py-3 rounded-lg font-bold transition-all ${selectedAnswer === null
                                ...
                                : 'bg-gradient-to-r from-neon-blue to-neon-purple text-white shadow-lg hover:shadow-cyan-500/30'
                                }`}
                        >
                            Submit Answer
                        </motion.button>
